...
class EmploymentsService @Inject()(validateRequestService: ValidateRequestService) {
  ...
  def validateCreateUpdateEmploymentExpenses(implicit request: Request[JsValue], APINumber: Int): Either[Result,Boolean] = {
    validateRequestService.validateRequest(ErrorModel(400,ErrorBodyModel("SCHEMA_ERROR", "The request body provided does not conform to the CreateUpdateEmploymentExpensesSchema.")), APINumber)
  }
}
